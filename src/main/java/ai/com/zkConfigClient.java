...
/**
 * @ Author     ：maoyeqin
 * @ Date       ：Created in 16:04 2018/10/12
...
public class ConfigClient {
    ...
    public ConfigClient(String connectString,String... types) {
    ...
    private void dump(String serverType) {
        ...
        try {
            ...
            List<String> children = zkClient.getChildren(parent, true, null);// 注册watch
            if (children == null || children.isEmpty()) {
                return;
            }
            Set<String> snap = new HashSet<String>();;
            for (String path : children) {
                byte[] data = zkClient.getData(parent + "/" + path, true, null);
    ...
    class InnerZK implements Watcher {
        ...
        public void process(WatchedEvent event) {
            ...
            switch (event.getState()) {
                case SyncConnected:
                    System.out.println("ConfigClient watch------Connected...");
                    init = false;
            ...
            try {
                ...
                while (true) {
                    ...
                    if (zkClient == null || (zkClient.getState() == States.NOT_CONNECTED || zkClient.getState() == States.CLOSED)) {
                        ...
                        try {
                            // session重建等异常行为
                            zkClient = new ZooKeeper(connectString, 10000, dw, true);
